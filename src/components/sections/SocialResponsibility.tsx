...
export function SocialResponsibility() {
  ...
  useGSAP(() => {
    if (document.querySelector('.initiative-card')) {
      gsap.fromTo(
        ".initiative-card",
        { y: 30, opacity: 0 },
        {
      ...
      <div className="container" ref={containerRef}>
        ...
        <div className="grid md:grid-cols-3 gap-8 3xl:gap-12 4xl:gap-16">
          {initiatives.map((item, index) => (
            <div 
              key={index} 
              className="initiative-card group p-10 md:p-12 3xl:p-16 4xl:p-20 bg-muted border border-primary/5 hover:border-accent/30 transition-all duration-500 hover:-translate-y-2 rounded-[3rem] rounded-tl-3xl"
            >
